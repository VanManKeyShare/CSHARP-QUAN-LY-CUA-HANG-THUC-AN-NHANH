...
namespace QUAN_LY_CUA_HANG_THUC_AN_NHANH
{
    public partial class Frm_DSDH : Form
    {
        ...
        private void LAY_DS_DH_CHUA_THANH_TOAN()
        {
            ...
            vmk.MS_SQL_QUERY = "SELECT * FROM DON_HANG WHERE THOI_GIAN_THANH_TOAN IS NULL ORDER BY MA_DH ASC";
        ...
        private void LAY_DS_DH_DA_THANH_TOAN()
        {
            ...
            VMK_DAO_FOR_MS_SQL vmk = new VMK_DAO_FOR_MS_SQL();
            vmk.MS_SQL_CONNECTION_STRING = SQL_CONNECTION_STRING;
            vmk.MS_SQL_QUERY = "SELECT * FROM DON_HANG WHERE THOI_GIAN_THANH_TOAN IS NOT NULL ORDER BY MA_DH ASC";
            vmk.MS_SQL_PARAMETERS = vmk.CREATE_MS_SQL_PARAMETERS();
            vmk.MS_SQL_PARAMETERS.Clear();
            ArrayList KQ = vmk.MS_SQL_SELECT();
        ...
        private void LAY_DS_CHI_TIET_DON_HANG_THEO_MA_DH(string ma_dh)
        {
            ...
            VMK_DAO_FOR_MS_SQL vmk = new VMK_DAO_FOR_MS_SQL();
            vmk.MS_SQL_CONNECTION_STRING = SQL_CONNECTION_STRING;
            vmk.MS_SQL_QUERY = "SELECT (SELECT TEN_BAN FROM BAN_AN WHERE MA_BAN = DON_HANG_CHI_TIET.MA_BAN) AS TEN_BAN, (SELECT TEN_HH FROM HANG_HOA WHERE MA_HH = DON_HANG_CHI_TIET.MA_HH) AS TEN_HH, SO_LUONG, DON_GIA, THANH_TIEN FROM DON_HANG_CHI_TIET WHERE MA_DH = @MA_DH";
            vmk.MS_SQL_PARAMETERS = vmk.CREATE_MS_SQL_PARAMETERS();
            vmk.MS_SQL_PARAMETERS.Clear();
        ...
        private void btn_huydh_Click(object sender, EventArgs e)
        {
            ...
            VMK_DAO_FOR_MS_SQL vmk = new VMK_DAO_FOR_MS_SQL();
            vmk.MS_SQL_CONNECTION_STRING = SQL_CONNECTION_STRING;
            vmk.MS_SQL_QUERY = "DELETE FROM DON_HANG_CHI_TIET WHERE MA_DH = @MA_DH; DELETE FROM DON_HANG WHERE MA_DH = @MA_DH";
            vmk.MS_SQL_PARAMETERS = vmk.CREATE_MS_SQL_PARAMETERS();
            vmk.MS_SQL_PARAMETERS.Clear();
            vmk.MS_SQL_PARAMETERS.Rows.Add("@MA_DH", ma_dh, SqlDbType.Int);
